...
a machine can check. Ten steps exist today: `task → brd → survey-plan → scope → graph → intake →
weight → ripple → design → plan`. The rest is `docs/concept.md`.
$END_GOAL
...
- `ext/index.mjs` — the extension: host functions (`readText`, `answers`, `checkTask`, `checkBrd`,
  `promote`, `setPending`, `clearPending`, `design`, `plan`) plus the pi tool `izi_answer`. All disk work
...
`standards/runbox.md`. It carries what only live runs taught: the
