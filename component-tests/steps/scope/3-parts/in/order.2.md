$START_TASK
Map cell src~main~java~ai~labs~eddi~modules~llm~bootstrap of this repository: one `<module>` per file, one `<gap>` per file that is genuinely
...
$START_DATA
$START_DOCUMENT
files of cell src~main~java~ai~labs~eddi~modules~llm~bootstrap — the whole world of this run: these paths, and only these.
...
$START_FILE path=src/main/java/ai/labs/eddi/modules/llm/bootstrap/LlmModule.java (4409 b)
/*
...
$START_PREVIOUS
$START_DOCUMENT
path: .agent/staging/part~src~main~java~ai~labs~eddi~modules~llm~bootstrap.xml
...
path: .agent/staging/part~src~main~java~ai~labs~eddi~modules~llm~bootstrap.xml
schema:
  <part cell="src~main~java~ai~labs~eddi~modules~llm~bootstrap" kind="survey">
    <module path="…">
      <role>…</role>
...
check: the script judges the file you write at .agent/staging/part~src~main~java~ai~labs~eddi~modules~llm~bootstrap.xml by the part guardrail (grammar 4); a red verdict returns as FEEDBACK with rule numbers and paths
