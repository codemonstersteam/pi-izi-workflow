...
## T79: строки-ограничения заказа не теряются на нормализации (FRUIT-1 27.08, R3)

### Замер дефекта

Живой прогон FRUIT-1 (27.08, после T77/T78): frd.xml продвинут, но R3 «Существующие вызовы
ломать нельзя» отсутствует — `normalized.md` прогона содержит 2 строки вместо 3. Intake честен:
матрица и F11 строятся по brd, который уже усечён.

Две независимые дыры (обе от разведки):
1. **Наряд вёл мимо**: `order.normalize.tpl` «Name the things the request CREATES — one row
   each» — ограничение не «creates», модель честно не записала его; чистка скопировала
   («Never add a row» — и правильно).
2. **Страж слеп**: lost-value работает по literalsOf — латиница с внутренней заглавной, цифры,
   кавычки, URI; кириллическая проза литералом не становится (литералы того TASK = {ui, fruit, 1}).

### Правки

1. tpl: требование = и создание, и ОГРАНИЧЕНИЕ (замкнутые маркеры, оба языка); строка
   `preserve | … | … | <ограничение дословно>`; дословная цитата в values — якорь сверок.
2. `clean.mjs::constraintRows(task, table)` в judgeClean: предложение с маркером ограничения
   обязано иметь носителя — preserve/keep/maintain-строку ИЛИ строку, делящую ≥2 слова
   (строка-цитата). Общий «каждое предложение — строка» невозможен: таблица английская с
   русского заказа, лексического пересечения нет — покрыт именованный класс.
3. Юниты (кейс прогона / preserve / цитата / эталон eddi чист), компонентный сценарий 10
   (quarkus-constraint фикстура: 2 строки → наряд починки чистки с constraint-row),
   шов классов дополнен.

### Seam

Реинтродукция: убрать constraintRows из judgeClean — юниты и сценарий 10 краснеют ровно
на «гардрейл пропустил потерянное ограничение».
