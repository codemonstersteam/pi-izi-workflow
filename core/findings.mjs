...
// B1 (S9). `mechanism-without-adr` раньше жил здесь, в ADVICE_CODES, — но ADVICE_CODES заведён под
// правила БЕЗ резолвера истины (см. шапку файла), а у этого правила резолвер ЕСТЬ: имя механизма
// приходит не догадкой по естественному языку, а явным маркером `ИЗОБРЕТАЕТСЯ` секции 9
// (validate-design-graph.mjs строит `opts.invented` строго регуляркой `/ИЗОБРЕТАЕТСЯ/i` — то же
// самое узкое совпадение, никакого «или механизм» вольнее, см. G2 реестра находок про ripple.mjs,
// который так не делает и сюда не поставляет). Попадание в ADVICE_CODES было не «мягче для роли» —
// это делало правило НЕРАБОЧИМ: `@program-designer` обещает «section-9 mechanism without ADR = FAIL»
// (LINKS роли), а advice не роняет check и не вызывает пере-делегацию — граф без единого ADR
// проходил зелёным. Раз входной список `invented` по построению несёт только ТОЧНО маркированные
// механизмы, правило теперь blocker безусловно. Если когда-нибудь появится поставщик с нестрогим
// совпадением (по слову «механизм» без маркера) — тот код обязан остаться в ADVICE_CODES выше, а не
// сюда: судить формулировку естественного языка эта функция не может.
