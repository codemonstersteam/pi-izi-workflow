...
// ОТВЕТ ОПЕРАТОРА ПЕРЕЖИВАЕТ ПЕРЕСБОРКУ ПЛАНА. Он уезжает в журнал решений и оттуда попадает в
// собранный документ; без журнала он жил бы только в тексте артефакта и умирал при первой же
// пересборке — так за три дня трижды терялась одна и та же работа (docs/plan.md Ш9).
test("полоса: вопрос роли — рельса, и ответ оператора ложится в журнал решений", () => {
  const loop = WORKFLOW.slice(WORKFLOW.indexOf("async function portions("))
  assert.match(loop, /env\.kind === "question"/, "вопрос роли обработан как ошибка, а не как рельса")
  assert.match(loop, /await askOperator\(env, q\.n,/, "вопрос не доезжает до оператора")
  assert.match(loop, /await decision\(\{ question: env\.subject,[\s\S]{0,200}route: "operator"/,
    "ответ оператора не попадает в журнал решений — он умрёт при первой пересборке плана")
  assert.match(loop, /n--;/, "после ответа порция не переписывается")
})
...
// РАЗБОР — ЕЩЁ НЕ ЗАПУСК. `new Function` проверяет СИНТАКСИС и молчит о связывании: полоса с
// потерянной константой разбирается прекрасно и умирает в первую секунду живого прогона.
//
// BUG_FIX_CONTEXT: 21.08.2026. При переписывании шага 9 из файла исчезло определение `NO_FEEDBACK`,
// оставшись в трёх местах употребления. Тест «валидный JavaScript» был зелен, прогон встал сразу
// после первого скрипта: «NO_FEEDBACK is not defined». Сторожится ШАГ 9 — код, который переписан
// последним: его константы обязаны быть объявлены в этом же файле, всё остальное приезжает
// глобалями хоста и объявлено в шапке полосы.
test("полоса: константы шага 9 объявлены в этом же файле, а не только употреблены", () => {
  const step9 = WORKFLOW.slice(WORKFLOW.indexOf("async function designing("))
    .split("\n").filter((l) => !l.trim().startsWith("//")).join("\n")
    .replace(/`(?:\\[\s\S]|[^`\\])*`/g, "``")
    .replace(/"(?:\\.|[^"\\])*"/g, '""')
    .replace(/\b[A-Z][A-Z0-9_]*\s*:/g, "k:")
  const used = new Set([...step9.matchAll(/\b([A-Z][A-Z0-9_]{2,})\b/g)].map((m) => m[1]))
  const declared = new Set([...WORKFLOW.matchAll(/\b(?:const|let|var)\s+([A-Z][A-Z0-9_]{2,})\b/g)].map((m) => m[1]))
  const orphans = [...used].filter((x) => !declared.has(x))
  assert.deepEqual(orphans, [], `константа шага 9 употреблена, но не объявлена — прогон умрёт на первой строке: ${orphans.join(", ")}`)
  // И сама потеря воспроизводима: без объявления шов обязан краснеть.
  assert.ok(declared.has("NO_FEEDBACK") && declared.has("STEP9") && declared.has("STAGING_DIR"))
})
