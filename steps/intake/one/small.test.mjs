...
// Fixtures are strings here: a minimal mapxml and a brd — the two etalons (quarkus 9/3, eddi
// 1854/16) are what CALIBRATED the constants, they are not needed to test them.
...
const nine = Array.from({ length: 9 }, (_, i) => `src/pkg/Mod${i}.java`)

test("калибровка quarkus: 9 узлов карты и 3 R-строки → маленькая, один вызов", () => {
  const cwd = stand(nine, BRD3)
  try {
    assert.equal(isSmall({ cwd }), true, "9/3 — внутри обоих порогов, а трек выбран полный")
  } finally { rmSync(cwd, { recursive: true, force: true }) }
})

test("13 узлов карты → полный путь (порог узлов)", () => {
  const cwd = stand(Array.from({ length: 13 }, (_, i) => `src/pkg/Mod${i}.java`), BRD3)
  try {
    assert.equal(isSmall({ cwd }), false, "13 > 12 узлов — карта уже не помещается в один наряд")
  } finally { rmSync(cwd, { recursive: true, force: true }) }
})

test("6 R-строк → полный путь (порог требований)", () => {
  const brd6 = "R1 один\nR2 два\nR3 три\nR4 четыре\nR5 пять\nR6 шесть\n"
  const cwd = stand(nine, brd6)
  try {
