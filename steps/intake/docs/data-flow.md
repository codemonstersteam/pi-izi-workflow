...
isSmall(state)?
  ...
  └─ ДА  → say «маленькая задача: один вызов», portions = [one] (staging frd~one.xml;
           ...
           красный ЧИНИМЫЙ (T77) → КРУГ ПОЧИНКИ, как у всякого пласта (T44): round+1,
                     blockers = FEEDBACK, previous = свой frd~one.xml — модель правит
                     названное, не переписывая хорошую работу. Цена красного-чинимого
                     2-3 вызова, не 1+6 (живой круг 27.08: дыра в одну строку — R3 без
                     владельца — стоила уничтожения 95%-готового артефакта; за это T77)
           красный ПОСЛЕ ИСЧЕРПАНИЯ кругов (loops) → ПАДЕНИЕ НА ПОЛНЫЙ ПУТЬ: portions =
                     шесть пластов, все todo и round 1 (круг головы НЕ тратится); блокеры
                     разносятся по passOfBlocker — код правила даёт пласт (F3c → contracts,
                     F5 → data-failures, …), rtm:-строки → owners (маршрут coverage), строки
                     без распознанного кода → scenarios с пометкой [one]; черновик
                     frd~one.xml остаётся на диске. Исчерпание — честный сигнал «задача
                     не маленькая», а не потеря работы
```
...
frd.xml); быстрый путь не прячет дефект — красный чинится на месте (T77), а исчерпав
круги — уходит на полный путь.
