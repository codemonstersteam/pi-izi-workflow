...
export function fold(state, event = {}) {
  ...
  if (blockers) {
    // T77 — КРАСНАЯ ПОРЦИЯ one ЧИНИТСЯ НА МЕСТЕ, ПАДЕНИЕ — ПОСЛЕДНЕЕ СРЕДСТВО. Живой прогон
    // 27.08: один вызов дал артефакт, хороший на 95%, суд поймал ОДНУ дыру (R3 без владельца)
    // — а механика ответила уничтожением работы: полный путь с нуля, 1+6 вызовов. Это хуже
    // полного пути вовсе. Красный пласт везде получает КРУГ ПОЧИНКИ (T44: previous = свой
    // staging, blockers = FEEDBACK, round+1) — порция one не исключение: назовём владельца,
    // поставь <question> (R3 — классический кандидат: ask-рельса T64 ответит за круг).
    // ПАДЕНИЕ — только когда круги исчерпаны: это честный сигнал «задача не маленькая»,
    // а не потеря работы. Блокеры при падении разносятся ПО КОДУ ПРАВИЛА: passOfBlocker
    // даёт пласт строки; `rtm:`-строки едут на owners (маршрут coverage); нераспознанные —
    // scenarios с пометкой [one]. Круги у шести начинаются с 1.
    if (p.id === "one") {
      // бюджет кругов — loops (реальный предел ФОРМЫ порции: round > loops+1 форма отвергает),
      // не intakeLoops: тот_guard для порций недостижим
