...
//             `.agent/anchors.json::analogue.files` — файлы-образцы (T26): список посчитал шаг 2D,
//             карта только ПОМЕЧАЕТ ими узлы.
// Invariants: ТОТАЛЕН; отсутствие anchors.json — молчание пометки, не отказ (карта без образцов
//             законна: шаг 2 старой полосы мог не написать артефакт).
// Interface:  mergeOf
import { newGraph, graphXml } from "./graph.mjs"
import { plan, focus, readAt } from "./inputs.mjs"
import { COMPUTED, partAt } from "./paths.mjs"

const ANCHORS = ".agent/anchors.json"

function samplesOf(state) {
  try {
    const a = JSON.parse(readAt(state.cwd, ANCHORS) || "null")
    return a && a.analogue && Array.isArray(a.analogue.files) ? a.analogue.files : []
  } catch { return [] }
}
...
export function mergeOf(state) {
  ...
  const r = newGraph({ parts, computedXml: readAt(state.cwd, COMPUTED), plan: p.plan, focus: f.focus, samples: samplesOf(state) })
