...
THE REQUEST TRANSLATED INTO THE LANGUAGE OF CODE, one row per requirement, four columns:
`<verb> | <object> | <instrument> | <values>`. WRITE YOUR R LINES FROM THIS TABLE — one consequence
per row: what CHANGES in this repository if that row is granted. The `values` column is not your
work: a later step measures it (LAW 2).
$START_ROWS_CONTENT
{ROWS}
$END_ROWS_CONTENT
$END_ROWS_DOCUMENT
