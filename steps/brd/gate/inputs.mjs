...
// Purpose:    одно решение: есть ли из чего судить. Ворота судят ТАБЛИЦУ ДЕЙСТВИЙ подшага 2A, а не
//             сырой заказ: по ней роль пишет следствия, из её колонок посчитаны кандидаты в якоря.
// io:         fs
...
//                 между прогонами её правят руками, и следствия строились бы по документу, которого
...
export function inputs(state) {
  ...
  if (!existsSync(abs)) {
    return { cls: "no-normalized", why: `${NORMALIZED} не существует — ворота судят таблицу действий, а её пишет подшаг brd/normalize: он не закрыт` }
  }
  const text = readFileSync(abs, "utf8")
  if (!parseRows(text).length) {
    return { cls: "unreadable-normalized", why: `${NORMALIZED} не содержит ни одной строки таблицы «<verb> | <object> | <instrument> | <values>» — судить нечего: переиграй brd/normalize` }
  }
  const known = state.at && state.at.normalized
  if (known && known.sha1 !== sha1of(text)) {
    return { cls: "normalized-changed", why: `${NORMALIZED} изменился после того, как его продвинул brd/normalize — следствия нельзя писать по документу, которого больше нет: переиграй brd/normalize` }
  }
  return null
}
