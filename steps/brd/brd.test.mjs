...
// izi-pi-v2: роль живёт не рядом с ядром (steps/brd/role.md, как в доноре izi-flow-v2), а в
// roles/gilb.md (PLAN.md §3). S1 писал этот тест самопропускающимся, потому что файла роли ещё не
// было (S2 шла параллельно). Роль перенесена (S2 сдана) — самопропуск снят: standards/code.md
// («тест без запуска — не тест») не делает исключения для «пока файла нет».
const ROLE_PATH = join(HERE, "../../roles/gilb.md")
