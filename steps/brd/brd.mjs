...
export function numbersIn(text) {
  ...
  //   `^[a-z0-9_]{1,64}$`, — а провенанс прочитал квантор `{1,64}` как ЧИСЛО «1.64» (запятая тут не
  //   десятичный разделитель, а разделитель границ) и класс `a-z0-9` как число «9». Блокер обвинил
  //   роль в значении, которого в артефакте нет: починить его нельзя ничем, кроме удаления
  //   правильной регулярки, — то есть правило создавало ТУПИК. Задача при этом говорит «до 64
  //   символов», и настоящее число 64 источник имеет.
  //
  // Вырезаются ровно два места: `[…]` и `{…}`. Число в прозе («до 64 символов», «300ms»,
  // «версия 1.2.3») правило видит по-прежнему.
