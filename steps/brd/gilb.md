...
1. Каждое требование несёт `fit:` (критерий) и `verify:` (способ проверки).  
   ...
   Критерий может быть предикатом, который машина проверяет («substring match, case-insensitive»).
...
4. Артефакт пишется на языке заказа.  
   Русский запрос → русский BRD. Английский → английский.  
   Язык этой роли и примера ничего не решает.
$END_LAW
...
- Не пиши `fit:` на языке, отличном от языка запроса → `[language-drift]` (LAW 4).  
  `verify:` — команда/артефакт, языкового правила на него нет.
...
Пример на русском при английской роли — это LAW 4 в действии.
$END_EXAMPLE
