// MODULE_CONTRACT: judge — гардрейл шага 9B: голова над шестью судьями
...
// io:         none
// EXTERNAL_DEPENDENCY: judge/T1..T6 — по судье на правило; tree.mjs::parseTree — разбор дерева.
...
// (полнота решения), T2 (адреса) и T6 (seed → Changed: дельта своего модуля видна порции целиком).
// Состав, владелец типа и ацикличность — свойства ЦЕЛОГО, и порция их видеть не может по
...
//                   known — пути репозитория; seeds — Set<путь> seed-модулей ripple.xml }
//   Dependencies: T5, T2, T6
//   Antecedent:   —  (тотален: любой текст судится)
//   Consequent:   success: [] — порция принята; failure: блокеры
//   Purity:       pure
//   Interface:    judgePortion({ text, mine, kin, known, seeds }) -> string[]
export function judgePortion({ text = "", mine = [], kin = [], known = [], seeds = new Set() } = {}) {
  const r = read(text)
  if (r.blockers) return r.blockers
  return [...T5({ modules: r.modules, said: r.said, mine }), ...T2({ modules: r.modules, kin: kin.length ? kin : mine, known }), ...T6({ modules: r.modules, seeds })]
}

// FUNCTION_CONTRACT: judgeWhole — вердикт по склеенному дереву
//   Input:        { text — склейка; frd — разобранное требование; seeds — Set<путь> seed-модулей }
//   Dependencies: T1, T4, T3, T6
//   Antecedent:   —  (тотален)
//   Consequent:   success: []; failure: блокеры
//   Purity:       pure
//   Interface:    judgeWhole({ text, frd, seeds }) -> string[]
export function judgeWhole({ text = "", frd = {}, seeds = new Set() } = {}) {
  const r = read(text)
  if (r.blockers) return r.blockers
  return [...T1({ said: r.said, frd }), ...T4({ modules: r.modules, said: r.said }), ...T3({ modules: r.modules, said: r.said }), ...T6({ modules: r.modules, seeds })]
}
