...
$START_OUTPUT
path: {STAGING}
schema: файл пишется ЦЕЛИКОМ и с корнем — фрагменты <flow> без <flows> не принимаются
  <flows task="…">
    <flow id="{UC}" uc="{UC}" goal="…">
      <step n="1" module="<путь из твоего дерева>" in="<значение, которое приходит>" out="<значение, которое уходит>" role="порождаю" closes="{UC}/1"/>
    </flow>
  </flows>
check: {CHECK}
СДЕЛАЙ ЭТО СЕЙЧАС: пройди по находкам сверху вниз, поправь названные места и запиши файл ЦЕЛИКОМ — с
корнем `<flows>` и всеми потоками порции, включая те, к которым находок не было, — по staging-пути
инструментом `write`; только после этого вызови `workflow_result` по OUTPUT_FORMAT своей ROLE.
$END_OUTPUT
