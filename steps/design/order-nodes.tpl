...
- Значение с `closes="UC…"` стоит в контракте узла, который называет сценарий этого use case:
  `closes="UC1/in"` — в `in`, `closes="UC1/post"` и `closes="UC1/1a"` — в `out`. Иначе правило 13.
- У узла с `delta` в `out` есть хотя бы одно значение. Пустой `out` — правило 14: маршрут обязан
  пройти через этот узел, а шаг пишется значением из `out`.
