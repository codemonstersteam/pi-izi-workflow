...
5. **Строка `зовёт:` обязательна у каждого раздела и несёт ПУТИ ФАЙЛОВ либо слово «нет».**
   ...
   ```
   верно:    зовёт: src/backup/IResourceSource.java — читает источники
   неверно:  зовёт: source.readGlossaries · IResourceSource.readGlossaries · SourceData.glossary
   ```

   Что именно вызываешь — пиши словами после тире.

6. **Строка `закрывает:` — шаг берёт тот раздел, по которому ПИШЕТСЯ ТЕСТ этого шага.**
   ...
   зовёт: IGlossaryStore · GlossaryService      зовёт: IGlossaryStore
   ────────────────────────────────────────     ────────────────────────────────────
   тест:                                        тест:
     зови    deleteGlossary(id, version)          зови    delete(id, version)
     проверь GlossaryService.invalidateCache()    проверь ???  ← GlossaryService не в «зовёт»
...
7. **Номер шага КОПИРУЕТСЯ из заказа знак в знак.** Номера стоят в USECASES: `<step n="1">` даёт
   `шаг 1`, `<ext id="2a">` даёт `шаг 2a`. Букву не переводи: в заказе латинская `a` — пиши
   латинскую `a`, не русскую `а`. Каждый use case заказа назван хотя бы в одном разделе.
...
8. **Строка `проверка:` обязательна** — команда из CHECK и имя тест-класса. Без неё работу нечем
   закрыть.

9. Ничего не выдумывай сверх MODULES, NEIGHBOURS, SAMPLE и DRAFT: ни новых файлов, ни новых
