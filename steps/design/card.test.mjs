...
// «зовёт» — один интерфейс. Проверить это честно исполнителю было нечем, и слабая модель написала
...
test("правило 6: шаг не закрывают модулем, который не зовёт названного в шаге", () => {
  ...
  // Убрать ребро «зовёт» — и владельцу нечем проверить тот же шаг.
  const cut = GREEN_PART.replace("зовёт: src/model/Doc.java — принимает и отдаёт запись", "зовёт: нет")
  const B = part(cut).join("\n")
  assert.match(B, /UC2 шаг 1/)
...
// Роль пишет в «зовёт» то, что ВЫЗЫВАЕТ: `source.readGlossaries`. На отказ «такого файла нет» она
...
test("блокер отличает вызов от пути и говорит, что писать вместо него", () => {
  const call = GREEN_PART.replace("зовёт: src/model/Doc.java — принимает и отдаёт запись", "зовёт: source.readGlossaries")
  ...
  const ghost = GREEN_PART.replace("зовёт: src/model/Doc.java — принимает и отдаёт запись", "зовёт: src/model/Nope.java — нет такого")
