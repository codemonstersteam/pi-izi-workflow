...
export function checkValues({ staged, frd = {}, ripple = "" } = {}) {
  ...
  // СТРОКА, КОТОРУЮ ПОТЕРЯЛ ПАРСЕР, И СТРОКА, КОТОРОЙ НЕТ, — РАЗНЫЕ ДЕФЕКТЫ. Сырой `<` внутри
  // значения атрибута заканчивает элемент на себе (core/xml.mjs), и строка исчезает ДЛЯ ЧИТАТЕЛЯ,
  // оставаясь в файле. Сказать роли «строки v40 нет», когда она видит v40 своими глазами, — значит
  // отправить её переписывать то, что уже написано: круг за кругом до конца бюджета.
  //
  // BUG_FIX_CONTEXT: живой прогон 00d07e75 (20.08.2026), проход A. Скелет нёс
  //   `text="create(List&lt;T&gt; configs, …)"`, роль вернула `text="create(List<T> configs, …)"` —
  //   раскодировала при копировании. Четыре строки «пропали», блокер назвал их отсутствующими.
  const raw = String(staged || "")
  const unreadable = lost.filter((id) => new RegExp(`id="${id}"`).test(raw))
  const gone = lost.filter((id) => !unreadable.includes(id))
  if (unreadable.length) {
    B.push(`строки ${unreadable.join(", ")} в файле ЕСТЬ, но не читаются: внутри значения атрибута стоит сырой «<» либо «&», и элемент заканчивается на нём. Пиши «<» как &lt;, «>» как &gt;, «&» как &amp; — ровно так, как они стоят в скелете, откуда строка скопирована`)
  }
  if (gone.length) B.push(`строки ${gone.join(", ")} нет в файле — состав словаря считает скрипт, строки не добавляются и не удаляются: перепиши файл из скелета, заполнив text`)
